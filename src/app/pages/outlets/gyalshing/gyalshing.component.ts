...
@Component({
  selector: 'app-gyalshing',
  standalone: true,
  imports: [CommonModule, RouterModule, LazyLoadDirective],
  templateUrl: './gyalshing.component.html',
  styleUrls: ['./gyalshing.component.css']
})
export class GyalshingComponent implements OnInit, OnDestroy {
  ...
  private loadOutletDetail(): void {
    this.outletDetailService.getOutletDetail('gyalshing')
  ...
  onImageError(type: string): void {
    console.warn(`Failed to load ${type} image for Gyalshing outlet`);
  }
