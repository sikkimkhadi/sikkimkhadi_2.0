...
@Component({
  selector: 'app-jorethang',
  standalone: true,
  imports: [CommonModule, RouterModule, LazyLoadDirective],
  templateUrl: './jorethang.component.html',
  styleUrls: ['./jorethang.component.css']
})
export class JorethangComponent implements OnInit, OnDestroy {
  ...
  private loadOutletDetail(): void {
    this.outletDetailService.getOutletDetail('jorethang')
  ...
  onImageError(type: string): void {
    console.warn(`Failed to load ${type} image for Jorethang outlet`);
  }
