...
@Component({
  selector: 'app-singtam',
  standalone: true,
  imports: [CommonModule, RouterModule, LazyLoadDirective],
  templateUrl: './singtam.component.html',
  styleUrls: ['./singtam.component.css']
})
export class SingtamComponent implements OnInit, OnDestroy {
  ...
  private loadOutletDetail(): void {
    this.outletDetailService.getOutletDetail('singtam')
  ...
  onImageError(type: string): void {
    console.warn(`Failed to load ${type} image for Singtam outlet`);
  }
